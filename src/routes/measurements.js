...
router.get('/', async (req, res, next) => {
  try {
    const { rows } = await db.query(`${SELECT} ORDER BY m.time DESC`);
    res.json(rows);
  } catch (err) {
    next(err);
  }
...
router.get('/:id', async (req, res, next) => {
  try {
    const { rows } = await db.query(`${SELECT} WHERE m.id=$1`, [req.params.id]);
...
router.patch('/:id', async (req, res, next) => {
  ...
  try {
    const { rows } = await db.query(
      `UPDATE measurements SET ${updates.join(', ')}, updated_at=NOW() WHERE id=$${values.length} RETURNING *`,
      values
    );
    if (!rows.length) return res.status(404).json({ error: 'Measurement not found.' });
    res.json(rows[0]);
...
router.delete('/:id', async (req, res, next) => {
  try {
    const { rows } = await db.query('DELETE FROM measurements WHERE id=$1 RETURNING id', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Measurement not found.' });
    res.status(204).send();
  } catch (err) {
    next(err);
